...
def load_golden(data_file: Path, limit: int | None = None) -> list[dict]:
    rows: list[dict] = []
    if not data_file.exists():
        raise FileNotFoundError(f"Fichier dataset introuvable : {data_file}")
...
def collect_rag_responses(
    questions: list[dict], base_url: str, delay: float = 5.0, rag_timeout: float = 180.0
) -> list[dict]:
...
def main() -> None:
    ...
    parser.add_argument("--experiment", default="ragas-baseline", help="Nom de l'expérience MLflow")
    parser.add_argument(
        "--skip-ragas", action="store_true", help="Collecter seulement, sans évaluation Ragas"
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    parser.add_argument("--ragas-timeout", type=int, default=240, help="Timeout (s) par job Ragas")
    parser.add_argument(
        "--max-wait",
        type=int,
        default=15,
    ...
    parser.add_argument(
        "--rag-hyde", type=str, default=None, help="Valeur USE_HYDE du container RAG (true/false)"
    )
    parser.add_argument(
        "--rag-multi-query",
        type=str,
    ...
    with mlflow.start_run(run_name=run_label) as run:
        ...
        if args.from_responses:
            ...
            with open(args.from_responses, encoding="utf-8") as f:
                results = json.load(f)
            if args.limit:
            ...
            results = collect_rag_responses(
                questions, args.base_url, delay=args.delay, rag_timeout=args.rag_timeout
            )
            rag_seconds = time.perf_counter() - t0
            logger.info(
                "Collecte terminée en %.1fs — %d erreurs",
                rag_seconds,
        ...
        mlflow.log_params(
            {
                ...
                "use_query_normalization": args.rag_normalization
                or os.getenv("USE_QUERY_NORMALIZATION", "unknown"),
                "use_hyde": args.rag_hyde or os.getenv("USE_HYDE", "unknown"),
                "use_multi_query_expansion": args.rag_multi_query
                or os.getenv("USE_MULTI_QUERY_EXPANSION", "unknown"),
